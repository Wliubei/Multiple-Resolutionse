...
def get_unified_feature(mat, eval=False):
    ...
    mat = np.tile(mat, n_repeat)

    return mat[:MIN_N_FRAMES]
# def _prepare_network( filter_length=1724, hop_length=130, win_length=None, window='hann'):
#         stft = STFT(
...
class SpoofDataSet(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        utt_id = line[0]
        X = np.expand_dims(np.load(os.path.join(self.data_dir, f"{utt_id}.flac")), axis=0)   # [1, N, D]
...
class SpoofDataSetBalance(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # feat, sr = sf.read(os.path.join(self.data_dir, f"{utt_id}.flac"), dtype=np.float32)
        X = np.expand_dims(np.load(os.path.join(self.data_dir, f"{utt_id}.flac")), axis=0)   # [1, N, D]
        y = self.label_map[line[1]]   # target
        return utt_id, X, y
...
class SpoofDataSetBalanceSample(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feat, sr = sf.read(os.path.join(self.data_dir, f"{utt_id}.flac"), dtype=np.float32)
        # feat = preemphasis(feat, k=0.97).astype(np.float32)
        feat = np.squeeze(feat)
        feat = get_unified_feature(feat, eval=self.eval)
        X = np.expand_dims(feat, axis=0)
        # X = np.expand_dims(X, axis=0) # [1, N, D]
...
class SpoofDataLoaderBalanceSample(BaseDataLoader):
    def __init__(self, scp_file, data_dir, batch_size, shuffle=True, validation_split=0.0, num_workers=0, eval=False, read_protocol=False, protocol_file=None):
