...
class Trainer(BaseTrainer):
    ...
    # def _prepare_network(seft,filter_length=1724, hop_length=130, win_length=None, window='hann'):
    #     stft = STFT(
    ...
    def _train_epoch(self, epoch):
        ...
        """
        self.model.train()
        # stft = self._prepare_network(filter_length=1724, hop_length=130, window='blackman')
        total_loss = 0
        total_metrics = np.zeros(len(self.metrics))
        for batch_idx, (_, data, target) in enumerate(self.data_loader):
            data,target = data.to(self.device), target.to(self.device)
            # data, phase = stft.forward(data)
